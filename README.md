...
Αφαιρέθηκαν πριν το preprocessing χαρακτηριστικά όπως ids, weights και survey_id γιατι στα μοντέλα δεν χρειάζονται αυτα. Το weights βάση εκφωνησης υπάρχει μέσα στις μεταβλητές που χρησιμοποίηθηκαν για να υπολογιστεί (οπότε εναλλακτικά θα μπορούσε να αφαιρεθούν τα άλλα και να κρατηθεί το weights, κάτι που θα μείωνε και το πλήθος χαρακτηριστικών).
...
Από άποψη χρόνου εκτέλεσης ο XGBoost ήταν ο πιο χρονοβόρος, μετά το Random Forest, αναλόγως και των τιμών που δίνοταν στο tuning, γενικότερα όσο αυξανόταν πχ το βάθος του δέντορυ, τόσο αυξανόταν και ο χρόνος. Μετά ήταν το MLP και πιο γρήγορο ήταν το Linear Regression.
...
## Linear Regression: 
Είναι απλό και ερμηνεύσιμο μοντέλο, ωστόσω δεν μοντελοποιεί μη γραμμικές σχέσεις με αποτέλεσμα να επηρρεάζεται σημαντικά η απόδοσή του και τέλος υποθέτει ότι τα δεδομένα είναι ανεξάρτητα μεταξύ τους, κάτι που συχνά δεν ισχύει. Για αυτό και αναμένεται να παρουσιάσει χαμηλή απόδοση σε πιο περίπλοκα και πραγματικά δεδομένα.

## Random Forest:
Το Random Forest βασίζεται στο bagging, όπου κάθε δέντρο εκπαιδεύεται σε διαφορετικό τυχαίο υποσύνολο των αρχικών δεδομένων (κάθε υποσύνολο έχει ίδιο πλήθος δεδομένων με το αρχικό) ενώ επίσης κάθε δέντρο χρησιμοποιεί διαφορετικά χαρακτηριστικά. 
Με αυτόν τον τρόπο, τα επιμέρους δέντρα εκπαιδεύονται σε διαφορετικά δεδομένα και χαρακτηριστικά και έτσι το Random Forest είναι πιο ανθεκτικό στο overfitting, σε σχέση με ένα μεμονωμένο δέντρο απόφασης, ενώ επίσης έτσι μπορεί και μαθαίνει μη γραμμικές και άρα πιο σύνθετες σχέσεις. Επιπλέον μπορεί να εντοπίσει και να βγάλει σαν αποτέλεσμα ποιο χαρακτηριστικό έιναι πιο σημαντικό (feature importance).
Ωστόσο, ακριβώς λόγω αυξημένης πολυπλοκότητας είναι λιγότερο ερμηνεύσιμο από το Linear Regression.
...
Τέλος, λόγω της φύσης του ensemble, είναι λιγότερο ερμηνεύσιμο από απλούστερα μοντέλα, όπως το Linear Regression.
...
Επιπλέον, εφαρμόζει κλάδεμα δέντρων βελτιώνοντας έτσι τη γενίκευση. Όλα τα παραπάνω το καθιστούν ισχυρό μοντέλο για πολύπλοκα και μη γραμμικά δεδομένα.
Παρόλα αυτά όμως η αυξημένηππλυπλοκότητα, συνεπάγεται πιο πολλές υπερπαράμετρους και άρα δυσκολότερο (και πιο αργό) tuning, ενώ όπως και το Random Forest είναι δύσκολο στην ερμηνεία των αποτελεσμάτων του.
...
Αυτό θα επιλεχθεί στη συνέχεια για την τελική δοκιμή στα σύνολα επικυρωσης (test) καθώς στόχος είναι τα rates, στα οποία υψηλότερο σκορ έχει το MLP.
...
Συνολικά, τα αποτελέσματα δείχνουν ότι τα μη γραμμικά μοντέλα (Random Forest, XGBoost, MLP) υπερέχουν του γραμμικού λόγω του ότι πρόκειται για πρόβλημα πραγματικού κόσμου με μη γραμμικά δεδομένα και περίπλοκες σχέσεις.
...
### Θα βοηθούσε πιθανώς
- Καθαρισμός και μείωση θορύβου (να μην εχει ελλιπείς τιμές από την αρχή).  
- Περισσότερα δείγματα για καλύτερη εκπαίδευση των μοντέλων γιατί χωρίζονται και εξαρτώνται από surveys.  
- Πιθανως αν ειχε και δεδομένα ανά survey σαν επιπλέον πληροφορία επειδή γίνεται ανά survey
- Να υπήρχαν labeled test data

---
<img width="825" height="440" alt="Στιγμιότυπο οθόνης (1081)" src="https://github.com/user-attachments/assets/65ba923d-4a63-4ccb-b05d-c39988110535" />
<img width="756" height="567" alt="Στιγμιότυπο οθόνης (1082)" src="https://github.com/user-attachments/assets/0fba01a6-c490-4496-aa56-11c28c56de68" />
